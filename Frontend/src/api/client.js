...
export function getErrorMessage(data, fallback = "Ocurrió un error inesperado") {
  ...
  if (Array.isArray(detail)) {
    return detail
      .map((item) => {
        if (!item?.msg) return JSON.stringify(item);

        // "loc" suele ser algo como ["body", "price"] -- nos quedamos
        // con el nombre del campo (ignorando "body"/"query") para que
        // el mensaje diga QUÉ campo falló, no solo que algo falló.
